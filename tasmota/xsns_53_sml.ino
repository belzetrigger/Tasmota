...
#define WGS_COMBO 13
#define EBZD_G 14
#define EHZ_I 15     //BLZ: Ehz I Vattenfall
#define EHZ_I_GAS 16 //BLZ: Ehz I Vattenfall + Gas
#define EHZ_I_GAS_H2O 17 //BLZ: Ehz I Vattenfall + Gas

// select this meter
#define METER EHZ_I_GAS_H2O
...
struct METER_DESC const meter_desc[METERS_USED] = {
    [0] = {3, 's', 0, SML_BAUDRATE, "SML", -1, 1, 0}, // hardeware serial
    [1] = {14, 'c', 0, 50, "Gas"}                     // GPIO14 // D 5 gas counter
};
...
struct METER_DESC const meter_desc[METERS_USED] = {
    [0] = {3, 's', 0, SML_BAUDRATE, "SML", -1, 1, 0},  // hardeware serial
    [1] = {14, 'c', 0, 50, "Gas"},                     // GPIO14 // D 5 gas counter
    [2] = {16, 'c', 0, 10, "H20"}             // GPI16 /D0 water counter
};
// [2] = {1, 'c', 0, 10, "H20", -1, 1, 0}             // GPIO1 water counter
...
const uint8_t meter[] =
    ...
    // *** h2o
    //
    //"3,1-0:1.8.0*255(@100," D_GasIN ",cbm," DJ_COUNTER ",2|";

    // TODO 2 (@100) or 4 @10000
    // ----------------------------Wasserzähler--sensor53 c1------------------------------------
    // "1,=h==================|"
    "3,1-0:1.8.0*255(@100," D_H2oIN ",cbm," DJ_COUNTER ",2"; // 1
...
public:
  ...
  void set_pga(enum ads1115_pga val)
  {
...
{
  uint32_t meters;
  //AddLog_P2(LOG_LEVEL_INFO, PSTR("x"));
  for (meters = 0; meters < meters_used; meters++)
  {
    if (meter_desc_p[meters].type != 'c')
    {
      //  AddLog_P(LOG_LEVEL_INFO, PSTR("SML poll"));
  ...
  //AddLog_P2(LOG_LEVEL_INFO, PSTR("BLZ: smltbuf: %s"), smltbuf );
}
...
void SML_Decode(uint8_t index)
{
  ...
  while (mp != NULL)
  {
    ...
    if (mindex < 0 || mindex >= meters_used)
      mindex = 0;
    mp += 2;
    if (*mp == '=' && *(mp + 1) == 'h')
    {
        ...
        else
        {
          ...
          else
          {
            ...
            if (*mp == 'i')
            {
              ...
              if (lowByte(crc) != smltbuf[mindex][7])
                goto nextsect;
              if (highByte(crc) != smltbuf[mindex][8])
                goto nextsect;
              dval = mbus_dval;
...
void SML_Show(boolean json)
{
  ...
  if (lastmind < 0 || lastmind >= meters_used)
    lastmind = 0;
  while (mp != NULL)
  {
    // setup sections
...
#ifdef USE_DOMOTICZ
  //AddLog_P2(LOG_LEVEL_INFO, PSTR("xxx: domoticz %s") );

  // P1 Smart Meter {"idx":48,"nvalue":0,"svalue":"178000;0;0;0;400;0","Battery":100,"RSSI":7}

  // general akt. verbrauch; stand
  // {"idx":47,"nvalue":0,"svalue":"400;17000","Battery":100,"RSSI":7}

  if (json && !tele_period)
  {
    char str[16];

    //BLZ: leider nicht anhand des textes - also direkt feld ansprechen
    // Zählerstand          dtostrfd(meter_vars[0], 1, str);
    // aktu. Verbrauch:     dtostrfd(meter_vars[1], 1, str);
    // P1
    ...
    char usage1[16];
    dtostrfd(meter_vars[0], 1, usage1);
    int power = meter_vars[1];

    if (power != 0)
    {
      DomoticzSensorPowerEnergy(power, usage1);
      DomoticzSensorP1SmartMeter(usage1, "", "", "", power);
...
void SML_Init(void)
{
  ...
  AddLog_P(LOG_LEVEL_INFO, PSTR("xxx: SML INIT"));
...
#ifdef USE_SCRIPT
  //AddLog_P(LOG_LEVEL_INFO, PSTR("SML use script"));
  for (uint32_t cnt = 0; cnt < MAX_METERS; cnt++)
  {
  ...
  for (uint8_t meters = 0; meters < meters_used; meters++)
  {
    if (meter_desc_p[meters].type == 'c')
    {
      ...
      else
      {
        ...
        AddLog_P(LOG_LEVEL_INFO, PSTR("SML counters, set to input with pullup"));
    ...
    else
    {
      // serial input, init
#ifdef SPECIAL_SS
      //AddLog_P(LOG_LEVEL_INFO, PSTR("SML SPECIAL_SS"));
...
#else
      //AddLog_P(LOG_LEVEL_INFO, PSTR("SML No SPECIAL_SS"));
      meter_ss[meters] = new TasmotaSerial(meter_desc_p[meters].srcpin, meter_desc_p[meters].trxpin, 1);
#endif
      if (meter_ss[meters]->begin(meter_desc_p[meters].params))
      {
        //AddLog_P(LOG_LEVEL_INFO, PSTR("SML flush"));
...
void SML_Counter_Poll(void)
{
  ...
  //AddLog_P(LOG_LEVEL_INFO, PSTR("SML counter poll"));
  int f = digitalRead(16);
  //AddLog_P(LOG_LEVEL_INFO, PSTR("SML counter poll"));
  AddLog_P2(LOG_LEVEL_INFO, PSTR("BLZ >> %i"),f);
  for (meters = 0; meters < meters_used; meters++)
  {
    if (meter_desc_p[meters].type == 'c')
    {
      ...
      if (meter_desc_p[meters].params > 0)
      {
        if (ctime - sml_counters[cindex].sml_cnt_last_ts > meter_desc_p[meters].params)
        {
          ...
          if (meter_desc_p[meters].flag & 2)
          {
            ...
            {
              int16_t val = adc.read_sample();
              if (val > sml_counters[cindex].ana_max)
                sml_counters[cindex].ana_max = val;
              if (val < sml_counters[cindex].ana_min)
                sml_counters[cindex].ana_min = val;
        ...
        if (cindex == 0)
          SetDBGLed(meter_desc_p[meters].srcpin, DEBUG_CNT_LED1);
#endif
#ifdef DEBUG_CNT_LED2
        if (cindex == 1)
          SetDBGLed(meter_desc_p[meters].srcpin, DEBUG_CNT_LED2);
#endif
      }
      else
      {
...
bool XSNS_53_cmd(void)
{
  ...
  if (XdrvMailbox.data_len > 0)
  {
    ...
    else if (*cp == 'c')
    {
      ...
      ResponseTime_P(PSTR(",\"SML\":{\"CMD\":\"counter%d: %d value:%d\"}}"), index, RtcSettings.pulse_counter[index - 1]);
    }
    else if (*cp == 'r')
    {
      // restart
...
bool Xsns53(byte function)
{
  ...
  case FUNC_EVERY_50_MSECOND:
    // AddLog_P2(LOG_LEVEL_INFO, PSTR(">> %s"),function);
    if (dump2log)
      Dump2log();
    else
      SML_Poll();
